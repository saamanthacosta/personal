---
name: ponytail
description: Use /ponytail to force the laziest solution that works: YAGNI, stdlib, native, one line. Levels: lite, full (default), ultra. Triggered by "be lazy", "simplest solution", "yagni", or "do less".
license: MIT
compatibility: Model-only skill — operates on the current task context.
metadata:
  author: personal
  version: "0.1"
---
...
Persistent mode: ACTIVE EVERY RESPONSE. No drift back to over-building. Still active if unsure. Off only: "stop ponytail" / "normal mode". Default: **full**. Switch: `/ponytail lite|full|ultra`.
...
- `level` (optional): `lite`, `full`, or `ultra`. Defaults to `full`. The current session's level persists until changed or the session ends.

## Guardrails

- Ponytail governs what you build, not how you talk (pair with a terse-prose mode for terse prose).
- "stop ponytail" / "normal mode" reverts. Level persists until changed or session end.
...
| `ponytail-help` | mentions | by name (slash) |
