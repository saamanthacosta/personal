---
name: create-skill
description: Create a reusable `SKILL.md` for the personal `.agents/skills/` directory, generalizing a methodology observed in the conversation or from an explicit user request. Use ONLY when the user explicitly asks to author or capture a new skill (for example, "create a skill for X", "make this workflow reusable").
...
- Do not auto-create related customizations; surface them as suggestions only.
- Keep `create-skill` opt-in: `disable-model-invocation: true` ensures it only runs when the user asks.
