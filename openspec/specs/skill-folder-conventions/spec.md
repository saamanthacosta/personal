# skill-folder-conventions Specification

## Purpose
Define the canonical three-folder layout (`scripts/`, `references/`, `assets/`) that every shared skill in `.agents/skills/<name>/` SHALL follow, and the rules for classifying a file into one of the three folders.
## Requirements
...
- [[../changes/archive/2026-08-04-merge-pr-description-skills/proposal|merge-pr-description-skills (2026-08-04)]] — Add the mode-dispatch rule that allows a single `SKILL.md` to dispatch between mode-specific `references/` notes (used by `pr-description`).
