# research-spike Specification

## Purpose
Bounded investigate-and-report skill. Produces a synthesis note (YAML frontmatter + fixed section template) plus a bibliography delta (BibTeX block + Obsidian wikilinks) in the user's Obsidian vault. Composes with `latex-paper-draft` via path-based hand-off.
